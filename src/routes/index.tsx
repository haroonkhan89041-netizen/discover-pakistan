...
const cultureImages: ReadonlyArray<readonly [string, string]> = [[images.culture,"Traditional architecture"],[images.food,"A table made for sharing"],[images.lahore,"Mughal artistry"],[images.desert,"Desert traditions"],[images.city ?? images.karachi,"Modern expression"]];
...
function DiscoverPakistan() {
 ...
 return <div ref={root} className="bg-background text-foreground selection:bg-primary selection:text-primary-foreground">
  ...
  <main>
   ...
   <section className="px-5 py-24 lg:px-10 lg:py-32"><div className="reveal mx-auto mb-14 max-w-[1500px] text-center"><p className="text-xs uppercase tracking-[.3em] text-primary">A visual journey</p><h2 className="mt-4 font-display text-5xl md:text-7xl">Infinite Pakistan.</h2></div><div className="mx-auto columns-1 gap-3 space-y-3 md:columns-2 lg:columns-3 max-w-[1500px]">{[...destinations.slice(0,9)].map(([name,,img],i)=><figure onClick={()=>setLightbox(img)} className="reveal premium-hover group relative cursor-zoom-in break-inside-avoid overflow-hidden rounded-lg" key={`${name}-gallery`}><img loading="lazy" className={`cinematic-image w-full object-cover transition-transform duration-700 group-hover:scale-105 ${i%3===0?"h-[520px]":"h-[340px]"}`} src={img} alt={name}/><figcaption className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-background p-6 pt-20 font-display text-2xl opacity-0 transition-opacity group-hover:opacity-100">{name}</figcaption></figure>)}</div></section>
